...
extern void IRAM_ATTR __digitalWrite1(uint8_t pin, uint8_t val)
{
    ledcDetachPin(pin);
    if(val) {
        if(pin < 32) {
...
extern int IRAM_ATTR __digitalRead1(uint8_t pin)
{
    if(pin < 32) {
        return (GPIO.in >> pin) & 0x1;
    } else if(pin < 40) {
...
extern void digitalWrite1(uint8_t pin, uint8_t val) __attribute__ ((weak, alias("__digitalWrite1")));
extern int digitalRead(uint8_t pin) __attribute__ ((weak, alias("__digitalRead")));
extern int digitalRead1(uint8_t pin) __attribute__ ((weak, alias("__digitalRead1")));
