...
struct ledContext analogContext[8] = {
  { -1, 0, 1, 500},
  { -1, 0, 2, 500},
  { -1, 0, 3, 500},
  { -1, 0, 4, 500},
...
void IRAM_ATTR __analogWrite(uint8_t pin, uint16_t val)
{
    ...
    for (i=0; i < 7; i++)
    {
        if (analogContext[i].pin == pin || analogContext[i].pin == -1)
        {
            ledcAttachPin(pin, analogContext[i].channel);
            ledcSetup(analogContext[i].channel, analogContext[i].freq, 10);
            analogContext[i].pin = pin;
            analogContext[i].value = val;
            break;
        }
    }
    if(i>=7){
        ledcDetachPin(analogContext[0].pin);
        analogContext[0].pin = pin;
        analogContext[0].value = val;
        ledcAttachPin(pin, analogContext[0].channel);
        ledcSetup(analogContext[0].channel, analogContext[0].freq, 10);
        i=0;
    }
    ledcWrite(analogContext[i].channel, val);
}
